...
function Royal_Tab(element, undefined) {
    ...
    this._menu_scroller_width = 0;      // Number
    ...
    this._build = function() {
        ...
        // view scroller
        SELF._view_scroller = $('<div id="view-scroller">').addClass('scroller').append(_view_items).appendTo(SELF._views);
    ...
    this._openMenu = function(index, no_animation) {
        ...
        var _final_switch = function() {
            ...
            SELF._view_items.filter('.active_view_item').removeClass('active_view_item');
            next_view_item.addClass('active');
            next_view_item.addClass('active_view_item');
        };
        ...
        if (no_animation === true) {
            ...
            if (SELF._active_menu > index) {
                SELF._view_scroller.css('margin-left', '-10%');
                animate_to = '0%';
            }
    ...
    // open tab by index
    this._addOpenMenu2 = function(index, no_animation, isReload) {
        if (SELF._element.data('animating') === true) {
            return;
        }
    ...
    this._slideToMenu = function(index) {
        ...
        var menu_item_width  = menu_item.data('width');
        var menu_position    = SELF._getMenuPosition();

        if (!menu_item_right) {
            menu_item_right = SELF._menu_width - menu_item_left - menu_item_width;
    ...
    this._setSize = function() {
        ...
        var menuWidth = parseInt(SELF._menu_width);
        var outerWidth = parseInt(SELF._tabs.outerWidth()) - parseInt(SELF._dropdown_menu.width());
        // svar drdownWidth = parseInt(SELF._dropdown_menu.width());

        /*dropdown_menu 떨어지지 않도록 수정
        if (menuWidth > outerWidth) {*/
        if (menuWidth > outerWidth - 2) {  // 2 = border
    ...
    this.Events = {
        ...
        _menuClick: function(e) {
            ...
            if(menu_item.attr("id") === 'closeTab'){
            	return            	
            }else if (menu_item.hasClass('active') || menu_item.hasClass('disabled')) {
                return;
...
function Royal_Tab_Api(element, undefined) {
    ...
    this.duplicataionCheck = function(pid){
    	var dupchk = false;    	
    	$('.royal_tab .tabs .scroller').each(function(){
    		$(this).find('li').each(function(inx){
    			var current = $(this);    			
    			if(current.children().size() > 0) {
    				var emptyULtest = current.children().eq(0);    				
    	            if(emptyULtest.is('i')){    	            	
    	            	if(emptyULtest.attr("pid") == pid){
    	            		this._tab_index = inx;
    	            		dupchk= true; 
    	            		//alert("중복체크의 this._tab_index:::"+this._tab_index);
    	            	}    	            	
    	            }
    	        }
    	    });
    ...
    this.tabIndexFind = function(pid){
    ...
    /** 탭 추가후에 처리되어야 할 내용에 대해서 처리 **/
    this.addCallBack = function(index, selected, tab, content, isajax, pid, callMethod) {
    	this._tab_index = index;
    	    	
    	var duplicationCheck = this.duplicataionCheck(pid);
    	
    	if(!duplicationCheck){
    		
    		var new_menu_item = $('<li>').append(tab+'&nbsp;<i class="fa fa-minus-square" id="closeTab" pid="'+pid+'"></i>');
    		if (isajax) {
    		...
    		this._tab_index = this.tabIndexFind(pid); 
    	}
    	
    	//alert("마지막 this._tab_index:::"+this._tab_index);
    	
    	if (selected === true) {
    		CONSTRUCTOR._addOpenMenu2(this._tab_index, true, duplicationCheck);
    	}
    ...
    this.add = function(index, selected, tab, content, isajax, pid, callMethod) {

    	this._tab_index = index;

    	//alert("첫번째 _tab_index:::"+this._tab_index);
    	
    	var duplicationCheck = this.duplicataionCheck(pid);
    	
    	if(!duplicationCheck){
    		
    		if( CONSTRUCTOR._menu_items.length <= 6 ){		// add 2017.01.16 max tab count : 7 (20170116 change)sss
    			...
    			alert("Maximum tab count is 7. Please close the open tab.");
    		}
